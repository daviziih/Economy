...
public class Account {
	...
	public void save() {
		...
		try {
			...
			if (result.next()) {
				database.execute("update " + table + " set valor='" + this.balance.toPlainString() + "', toggle='"
						+ (this.toggle ? 1 : 0) + "' where name='" + this.name + "'");
			} else {
				database.execute("insert into " + table + " values ('" + this.name + "', '"
